...
| `0x71..0x74` | 轮胎红外温度 | 采集模块 | 完整图像就绪时触发 | **已接入** | 四轮各 4 个水平带平均温度（0.01 °C），按 ID 映射 FL/FR/RL/RR 写入 `thermal_summary`；映射已由采集端确认。 |
...
4. **轮胎温度轮位映射**：
   - CANB `0x71..0x74` 已接入 `thermal_summary`，按 `0x71=FL`、`0x72=FR`、`0x73=RL`、`0x74=RR` 映射四轮，每轮记录从上到下 4 个水平带平均温度。
