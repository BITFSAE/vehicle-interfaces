...
| `0x186250F4` | 温度极值与风扇 | 500 ms | **已接入** | 最高/最低温度 (原始值-30) 及探头编号 (0..47，上报时 +1)。`cooling_control` 已收存，风扇占空比/转速未编码。 |
| `0x186350F4` | 继电器与工作状态 | 500 ms | **已接入** | 包含正/负/预充继电器命令、充电状态、预充电压。固件不从此帧解析 `battery_state`；继电器/充电状态编码入 `bms_telemetry`，预充电压即自有 IVT-S U2（`ivt_telemetry.voltage_u2_mv`）。 |
| `0x186750F4` | 单体电压累加和 | 1 s | **已接入** | 138 串单体累加总压 (大端 0.1 V)。作为 `hv_voltage` 第 3 优先级备用源。 |
| `0x187650F4` | 故障汇总 | 500 ms/变发 | **已接入** | 32 位故障字 `FaultCode` 及从控离线掩码。编码入 `fault_code` 与 `battery_fault_code`。 |
...
| `0x187850F4` | 32 项告警等级表 | 2 s/变发 | 未接入 | 32 项告警触发等级（与 CANB `0x4B2` 冗余），待扩展至 `alarms[]`。 |
...
| `0x401/402/404/405` | Chroma 充电机反馈 | Chroma | 周期 | **已接入** | Chroma 充电电压/电流（LE float32）、保护位图与输出状态，编码入 `charger_telemetry`。 |
| `0x4A0/0x4A3/0x4A4` | BMS/ECU SOP 限值 | BMS/ECU | 50 ms | **已接入** | 充/放电流与功率上限、SOP 状态/确认标志，编码入 `sop_limits`。 |
| `0x4B2` | BMS 告警等级冗余 | BMS | 2 s | 未接入 | 32 项告警等级表（与 CAN1 `0x187850F4` 同格式冗余）。 |
| `0x5A0/0x5A1` | 低压配电 PDM | PDM | 周期 | **已接入** | 低压母线/蓄电池电压、电流、功率、能量，编码入 `pdm_telemetry`。 |
| `0x5A2..0x5A7` | 风扇控制器 | 风扇板 | 周期 | **已接入** | 风扇转速、PWM 占空比/目标/应答、故障与曲线/故障安全参数，编码入 `fan_telemetry`（0x5A4 命令帧不解析）。 |
| `0x515/0x516/0x518` | IVT U3/温度/As | IVT_S | 周期 | **不需要** | 当前遥测仅需 I/U1/U2/W/Wh 5 个核心物理量。 |
...
4. **轮胎温度轮位映射**：
   - CANB `0x071..0x074` 16 点已接入 `thermal_summary`，当前按 0x071=FL、0x072=FR、0x073=RL、0x074=RR 假设，待与数采模块确认。
