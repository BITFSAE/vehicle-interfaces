...
| `0x4A0/0x4A3/0x4A4` | BMS/ECU SOP 限值 | BMS/ECU | 10 ms（非充电模式） | **部分上报** | `0x4A0/0x4A3` 通过成组 CRC 和 100 ms 配对检查后才更新限值；`0x4A4` 独立验 CRC。完整状态保存在本地，Proto 已定义的限值和汇总标志进入 `sop_limits`。 |
